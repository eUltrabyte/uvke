...
namespace uvke {
    DepthBuffer::DepthBuffer(Base* base, Surface* surface)
        : m_base(base) {
        m_image = std::make_unique<Image>(m_base, vec2i(surface->GetExtent().width, surface->GetExtent().height), m_base->GetDepthFormat(), VK_IMAGE_USAGE_DEPTH_STENCIL_ATTACHMENT_BIT, base->GetSampleCount());
